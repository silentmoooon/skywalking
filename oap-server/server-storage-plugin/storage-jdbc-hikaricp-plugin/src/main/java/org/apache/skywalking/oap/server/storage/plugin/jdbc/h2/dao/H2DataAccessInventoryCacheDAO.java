...
public class H2DataAccessInventoryCacheDAO extends H2SQLExecutor implements IDatabaseAccessInventoryCacheDAO {
    ...
    @Override public int getServiceId(String serviceName) {
        String id = DatabaseAccessInventory.buildId(serviceName);
    ...
    @Override public int getServiceId(int addressId) {
        String id = DatabaseAccessInventory.buildId(addressId);
        return getEntityIDByID(h2Client, DatabaseAccessInventory.SEQUENCE, DatabaseAccessInventory.INDEX_NAME, id);
    }

    @Override public DatabaseAccessInventory get(int serviceId) {
        try {
            return (DatabaseAccessInventory)getByColumn(h2Client, DatabaseAccessInventory.INDEX_NAME, DatabaseAccessInventory.SEQUENCE, serviceId, new DatabaseAccessInventory.Builder());
        } catch (IOException e) {
    ...
    @Override public List<DatabaseAccessInventory> loadLastUpdate(long lastUpdateTime) {
        ...
        try {
            ...
            sql.append(" where ").append(DatabaseAccessInventory.DATABASE_TYPE_ID).append("=? ");
